...
function parseQueueRuntimeConfig(): QueueRuntimeConfig {
  ...
  if (isNaN(batchSize) || batchSize <= 0)
    throw new Error('QUEUE_BATCH_SIZE must be a positive number');
  ...
  if (isNaN(defaultMaxAttempts) || defaultMaxAttempts <= 0)
    throw new Error('DEFAULT_MAX_ATTEMPTS must be a positive number');
...
function parseWorkerConfig(): WorkerConfig {
  const v = (process.env.WORKER_MODE || '').toLowerCase();
  const enabled = v === '1' || v === 'true' || v === 'yes';

  return {
    enabled,
    natsSubjects: SubscriptionSubjects,
  };
}
