...
func (s *LeakScanService) ScanSnapshot(ctx context.Context, projectID, snapshotID string) ([]*domain.ClientLeakScan, error) {
	...
	if err != nil {
		metrics.Default.ObserveLeaks("scan", "error")
		return nil, err
	}
	...
	metrics.Default.ObserveLeaks("scan", "success")
	return findings, nil
}
...
func (s *LeakScanService) GetScanResults(ctx context.Context, projectID, snapshotID, severity string) ([]*domain.ClientLeakScan, error) {
	...
	if err != nil {
		metrics.Default.ObserveLeaks("results", "error")
	} else {
		metrics.Default.ObserveLeaks("results", "success")
	}
	return res, err
}
...
func (s *LeakScanService) MarkAsFixed(ctx context.Context, scanID string) error {
	...
	if err != nil {
		metrics.Default.ObserveLeaks("fix", "error")
	} else {
		metrics.Default.ObserveLeaks("fix", "success")
	}
	return err
}
