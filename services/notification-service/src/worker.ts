...
import { createRuntime, startNatsSubscriptions } from './bootstrap/runtime.js';
...
export async function bootstrap() {
  ...
  const templateLoader = createTemplateLoader({
    store: templateStore as any,
    fsBaseDir: cfg.templates.dir,
  });

  // Channels (email)
  const renderTemplate = createHandlebarsRenderer({
    templatesDir: cfg.templates.dir,
    loader: templateLoader as any,
  });
  ...
  const meter =
    otelMetrics.getMeterProvider?.().getMeter?.(cfg.observability.serviceName) ||
    ({ createCounter: () => ({ add: () => {} }) } as any);
  ...
  if (cfg.channels.twilioAccountSid && cfg.channels.twilioAuthToken && cfg.channels.twilioFrom) {
    ...
    const limiter = {
      tryRemoveTokens: () => {
        const now = Date.now();
        const refill = Math.floor((now - last) / 200);
        if (refill > 0) {
  ...
  const inappPub = createInAppPublisher({
    redis: redis as any,
    channelPrefix: 'inapp',
    repo: inappRepo as any,
    metrics,
  });
  adapters['in-app'] = createInAppChannel({
    publisher: inappPub as any,
    resolveToUserId: async (job: any) => job.userId,
  });
  ...
  if (cfg.channels.webhookUrl) {
    adapters['webhook'] = createWebhookChannel({
      http: (globalThis as any).fetch,
      url: cfg.channels.webhookUrl,
      signingSecret: cfg.channels.webhookSigningSecret,
      metrics,
      breaker,
      reliability: { timeoutMs: cfg.reliability.timeoutMs, retry: cfg.reliability.retry },
    });
  ...
  if (cfg.channels.slackWebhookUrl) {
    adapters['slack'] = createSlackChannel({
      http: (globalThis as any).fetch,
      webhookUrl: cfg.channels.slackWebhookUrl,
      metrics,
      breaker,
      reliability: { timeoutMs: cfg.reliability.timeoutMs, retry: cfg.reliability.retry },
    });
  ...
  const subscriptions = await startNatsSubscriptions(
    cfg.worker.natsSubjects,
    subscribe,
    runtime.router,
  );
  console.log(`Started ${cfg.worker.natsSubjects.length} NATS subscriptions`);

  const runloop = createRunLoop({
    dispatcher: runtime as any,
    batchSize: cfg.queue.batchSize,
    tickMs: cfg.queue.tickMs,
    logger: console,
  });
  runloop.start();
