	...
	m := &DomainMetrics{
		secrets: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				...
				Help: "Count of secret operations by op and outcome.",
			},
			[]string{"op", "outcome"},
		),
		parity: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Name: "parity_operations_total",
				Help: "Count of parity operations by op and outcome.",
			},
			[]string{"op", "outcome"},
		),
		leaks: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Name: "leak_operations_total",
				Help: "Count of leak-scan operations by op and outcome.",
			},
			[]string{"op", "outcome"},
		),
	}
...
func (m *DomainMetrics) ObserveSecrets(op, outcome string) {
	if m != nil {
		m.secrets.WithLabelValues(op, outcome).Inc()
	}
}

// ObserveParity increments parity counter for op/outcome.
func (m *DomainMetrics) ObserveParity(op, outcome string) {
	if m != nil {
		m.parity.WithLabelValues(op, outcome).Inc()
	}
}

// ObserveLeaks increments leak counter for op/outcome.
func (m *DomainMetrics) ObserveLeaks(op, outcome string) {
	if m != nil {
		m.leaks.WithLabelValues(op, outcome).Inc()
	}
}
