...
async fn test_get_dependency_vulns_returns_joined_rows() {
    ...
    let dep = dep_governance_service::models::Dependency::new(
        Uuid::new_v4(), "openssl", "1.1.1", dep_governance_service::models::Ecosystem::Cargo, true, None
    ).unwrap();
    ...
    let cve = dep_governance_service::db::vulnerabilities::upsert_cve(&pool, &cve).await.unwrap();
    let _link = dep_governance_service::db::vulnerabilities::link_vulnerability_to_dependency(
        &pool, dep.id, cve.id, "open", Some(">=1.1.0 <1.1.2"), Some("1.1.2")
    ).await.unwrap();

    let json = get_dependency_vulns_handler(Path(dep.id), State(pool)).await.unwrap();
    let list = json.0;
