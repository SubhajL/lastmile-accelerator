...
func (okSecretsHandler) CreateSecret(w http.ResponseWriter, r *http.Request) {
	handlers.Success(w, http.StatusCreated, map[string]any{"ok": true})
}
func (okSecretsHandler) ListSecrets(w http.ResponseWriter, r *http.Request) {
	handlers.Success(w, http.StatusOK, map[string]any{"ok": true})
}
func (okSecretsHandler) GetSecret(w http.ResponseWriter, r *http.Request) {
	handlers.Success(w, http.StatusOK, map[string]any{"ok": true})
}
func (okSecretsHandler) DeleteSecret(w http.ResponseWriter, r *http.Request) {
...
func TestIntegration_SecretsCreate_UnauthorizedWithoutJWT(t *testing.T) {
	...
	router := SetupRoutes(okSecretsHandler{}, nil, nil, nil, handlers.PanicRecovery(log), handlers.RequestLogger(log), handlers.JWTAuth(&fakeVerifier{allow: false}))
...
func TestIntegration_SecretsCreate_ForbiddenWithoutScope(t *testing.T) {
	...
	log := appLogger.New("test", "info", new(bytes.Buffer))
	router := SetupRoutes(okSecretsHandler{}, nil, nil, nil, handlers.PanicRecovery(log), handlers.RequestLogger(log), handlers.JWTAuth(ver))
	ts := httptest.NewServer(router)
	defer ts.Close()
...
func TestIntegration_RBAC_AugmentsScopes_AllowsWithoutDirectScope(t *testing.T) {
	...
	log := appLogger.New("test", "info", new(bytes.Buffer))
	router := SetupRoutes(okSecretsHandler{}, nil, nil, nil, handlers.PanicRecovery(log), handlers.RequestLogger(log), handlers.JWTAuth(ver), handlers.RBACAugment())
	ts := httptest.NewServer(router)
	defer ts.Close()
	...
	resp, _ := http.DefaultClient.Do(req)
	if resp.StatusCode != http.StatusCreated {
		t.Fatalf("want 201 with RBAC admin, got %d", resp.StatusCode)
	}
}
...
func TestIntegration_ParityCheck_ScopesAndPublish(t *testing.T) {
	...
	router := SetupRoutes(okSecretsHandler{}, parityH, nil, nil, handlers.PanicRecovery(log), handlers.RequestLogger(log), handlers.JWTAuth(verNo))
	ts := httptest.NewServer(router)
	defer ts.Close()
	...
	router2 := SetupRoutes(okSecretsHandler{}, parityH, nil, nil, handlers.PanicRecovery(log), handlers.RequestLogger(log), handlers.JWTAuth(verYes))
	...
	if len(cap.topics) == 0 || cap.topics[0] != "parity.check.completed" {
		t.Fatalf("expected publish parity.check.completed, got %v", cap.topics)
	}
}
...
func TestIntegration_LeakScan_ScopesAndPublish(t *testing.T) {
	...
	r1 := SetupRoutes(okSecretsHandler{}, nil, leakH, nil, handlers.PanicRecovery(log), handlers.RequestLogger(log), handlers.JWTAuth(verNo))
	...
	r2 := SetupRoutes(okSecretsHandler{}, nil, leakH, nil, handlers.PanicRecovery(log), handlers.RequestLogger(log), handlers.JWTAuth(verYes))
	...
	found := false
	for _, tpc := range cap.topics {
		if tpc == "leak.scan.completed" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected leak.scan.completed publish, got %v", cap.topics)
	}
}
