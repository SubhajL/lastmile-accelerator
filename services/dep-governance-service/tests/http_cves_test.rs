...
async fn test_upsert_cve_create_and_update() {
    ...
    let res = upsert_cve_handler(State(pool.clone()), Json(req)).await.unwrap();
    ...
    let res2 = upsert_cve_handler(State(pool.clone()), Json(req2)).await.unwrap();
...
async fn test_link_vuln_happy_and_conflict() {
    ...
    let (status, _) = link_vuln_handler(Path(dep.id), State(pool.clone()), Json(link_req)).await.unwrap();
