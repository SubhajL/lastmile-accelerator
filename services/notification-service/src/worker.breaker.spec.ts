...
vi.mock('./channels/circuitbreaker.js', () => ({
  createCircuitBreaker: vi.fn((opts) => { captured.push(opts); return { execute: (fn: any) => fn() }; })
}));
...
vi.mock('./config.js', () => ({
  loadConfig: vi.fn().mockReturnValue({
    ...
    auth: {},
    channels: { webhookUrl: 'https://example.com/hook', slackWebhookUrl: 'https://hooks.slack.com/xyz' },
    templates: { dir: 'src/templates' },
    queue: { batchSize: 10, tickMs: 200, defaultMaxAttempts: 3 },
    reliability: { timeoutMs: 5000, retry: { max: 1, baseMs: 1, jitterPct: 0 }, breaker: { failureThreshold: 7, windowSize: 11, halfOpenAfterMs: 12345 } }
  })
}));

vi.mock('./events/nats-subscribe.js', () => ({ createNatsSubscribe: vi.fn().mockReturnValue(() => ({ once: vi.fn().mockResolvedValue({ processed: 0, failed: 0 }) })) }));
vi.mock('./redis/client.js', () => ({ createRedisClient: vi.fn().mockReturnValue({ quit: vi.fn() }) }));
vi.mock('./notifications/queue.js', () => ({ createNotificationQueue: vi.fn().mockReturnValue({ dequeue: vi.fn(), ack: vi.fn(), nack: vi.fn() }) }));
vi.mock('nats', () => ({ connect: vi.fn().mockResolvedValue({ subscribe: vi.fn().mockReturnValue({}), close: vi.fn() }) }), { virtual: true });
vi.mock('ioredis', () => ({ default: vi.fn() }), { virtual: true });
vi.mock('./channels/renderer.js', () => ({ createHandlebarsRenderer: vi.fn().mockReturnValue(vi.fn().mockResolvedValue({ subject: 's', html: '<p/>', text: 't' })) }));
...
vi.mock('./bootstrap/runloop.js', () => ({ createRunLoop: vi.fn().mockReturnValue({ start: vi.fn(), stop: vi.fn() }) }));
