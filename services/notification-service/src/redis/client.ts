...
export interface CreateRedisClientOptions {
  RedisCtor?: new (url: string, options?: any) => IORedis;
...
export function createRedisClient(config: RedisConfig, opts: CreateRedisClientOptions) {
  ...
  const RedisClass = opts.RedisCtor as unknown as new (url: string, options?: any) => IORedis;
  const client = new RedisClass(config.url, {
    maxRetriesPerRequest: config.maxRetriesPerRequest
  });

  client.on('error', (err: unknown) => {
    try { opts.logger?.error?.('redis error', err); } catch {}
  });
